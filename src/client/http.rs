...
impl super::Client {
    ...
    pub async fn post_json<T: serde::Serialize>(&self, endpoint: &str, payload: &T) -> Result<Response> {
        let url = self.build_url(endpoint)?;
...
impl super::Client {
    ...
    /// Set mood lighting
    pub async fn set_mood_light(&self, brightness: Option<u8>, color: Option<Color>, kelvin: Option<u16>) -> Result<()> {
        let mut payload = serde_json::Map::new();
        
        if let Some(b) = brightness {
            payload.insert("brightness".to_string(), serde_json::Value::Number(b.into()));
        }
