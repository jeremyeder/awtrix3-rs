...
pub async fn execute(client: awtrix3::Client, command: DisplayCommands) -> Result<()> {
    match command {
        DisplayCommands::Mood { color, kelvin, brightness } => {
            // Validate kelvin range if provided
            if let Some(k) = kelvin {
                if k < 2000 || k > 6500 {
                    return Err(anyhow::anyhow!("Color temperature must be between 2000K and 6500K"));
                }
            }
            ...
            // Call API
            client.set_mood_light(brightness, parsed_color, kelvin).await?;
            
            // Provide feedback
            match (parsed_color, kelvin, brightness) {
                (Some(c), None, Some(b)) => println!("Mood light set to color {} with brightness {}", c.to_hex(), b),
                (None, Some(k), Some(b)) => println!("Mood light set to {}K with brightness {}", k, b),
        ...
        DisplayCommands::Screen { fps, fullscreen } => {
            ...
            println!("Would show screen at {} FPS{}", fps, if fullscreen { " (fullscreen)" } else { "" });
