...
pub async fn execute(client: awtrix3::Client, command: CustomCommands) -> Result<()> {
    match command {
        CustomCommands::Create { name, text, icon, duration, file } => {
        ...
        CustomCommands::List => {
            ...
            println!("Custom apps are managed through the web interface or by knowing their names.");
        }
        CustomCommands::Watch { name, file, interval } => {
            // Validate file exists
            if !std::path::Path::new(&file).exists() {
                return Err(anyhow::anyhow!("Configuration file not found: {}", file));
            }
            
            println!("Watching file '{}' for app '{}' (interval: {}s)", file, name, interval);
            ...
            loop {
                match fs::read_to_string(&file) {
                    Ok(content) => {
                        match serde_json::from_str::<CustomApp>(&content) {
                            Ok(app) => {
                                match client.create_custom_app(&name, &app).await {
